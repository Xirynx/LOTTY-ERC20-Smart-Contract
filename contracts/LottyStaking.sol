...
contract LottyStaking is Ownable {
	...
	event Stake(address indexed owner, uint256 indexed positionNonce, uint256 timestamp, uint256 liquidity);
	event Unstake(address indexed owner, uint256 indexed positionNonce, uint256 timestamp, uint256 liquidity);

    //============================================//
    //                 Constants                  //
    //============================================//

	IUniswapV2Pair constant uniswapV2Pair = IUniswapV2Pair(0x1840c51B131a51bb66F3019CC7B2d54e6d686E10);
	...
	function checkPosition(uint256 _nonce) public view returns (StakePosition memory _position) {
	...
	function stake(
		...
		positions[stakeNonce] = StakePosition(msg.sender, block.timestamp, liquidity, _timeLocked);
	...
	function unstake(
		...
		if (block.timestamp - position.timestamp < position.timeLocked) revert PositionLocked();
		...
		emit Unstake(msg.sender, _positionNonce, block.timestamp, position.liquidity);
	}
